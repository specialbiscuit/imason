...
function initMeScroll(mescrollId, options) {
	//下拉刷新的布局内容
	var htmlContent='<p class="downwarp-slogan">让购物更便捷</p>';
	htmlContent+='<p class="downwarp-text">下拉更新...</p>';
	htmlContent+='<div class="downwarp-progress">';
	htmlContent+='<img class="downwarp-man" src="option/mescroll-progress1.png"/>';
	htmlContent+='<img class="downwarp-box" src="option/mescroll-progress0.png"/>';
	htmlContent+='</div>';
	htmlContent+='<div class="downwarp-loading"></div>';
	
	//上拉加载中的布局
	var htmlLoading = '<div class="upwarp-progress"><div class="upwarp-loading mescroll-rotate"></div></div><p class="upwarp-tip">加载中..</p>';
	//无数据的布局
	var htmlNodata = '<p class="upwarp-nodata">~到底了~</p>';
	...
	var myOption={
		down:{
			...
			inited: function(mescroll, downwarp) {
				...
				mescroll.downManDom = downwarp.getElementsByClassName("downwarp-man")[0];
				mescroll.downBoxDom = downwarp.getElementsByClassName("downwarp-box")[0];
			...
			inOffset: function(mescroll) {
				...
				mescroll.downProgressDom.style.display="block";
			...
			outOffset: function(mescroll) {
				...
				mescroll.downLoadingDom.style.display="block";
			},
			onMoving: function(mescroll, rate, downHight) {
				//下拉过程中的回调,滑动过程一直在执行; rate下拉区域当前高度与指定距离的比值(inOffset: rate<1; outOffset: rate>=1); downHight当前下拉区域的高度
				if (rate<=1) {
					mescroll.downManDom.style.webkitTransform = "scale("+rate+","+rate+")";
					mescroll.downManDom.style.transform = "scale("+rate+","+rate+")";
					mescroll.downBoxDom.style.webkitTransform = "scale("+rate+","+rate+")";
					mescroll.downBoxDom.style.transform = "scale("+rate+","+rate+")";
				}
			},
